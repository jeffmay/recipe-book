...
describe("UserMenu", () => {
  it("renders the user name in the trigger", () => {
    render(<UserMenu user_name="Alice" on_profile={vi.fn()} />);
  ...
  it("calls on_profile when Profile settings is clicked", async () => {
    const on_profile = vi.fn();
    render(<UserMenu user_name="Alice" on_profile={on_profile} />);
    await userEvent.click(screen.getByLabelText("User menu for Alice"));
    await userEvent.click(screen.getByRole("menuitem", { name: "Profile settings" }));
    expect(on_profile).toHaveBeenCalledOnce();
  });
});
