...
import { make_fraction } from "@recipe-book/shared";
import type { Measurement } from "@recipe-book/shared";
import { MeasurementEditor } from "../MeasurementEditor.js";

const ONE_CUP: Measurement = { value: make_fraction(1, 1), unit: "cup" };
const ONE_OZ: Measurement = { value: make_fraction(1, 1), unit: "oz" };
const HALF_CUP: Measurement = { value: make_fraction(1, 2), unit: "cup" };
const WHOLE: Measurement = { value: make_fraction(3, 1), unit: "whole" };

function setup(measurement: Measurement = ONE_CUP, on_commit = vi.fn()) {
  render(<MeasurementEditor value={measurement} on_commit={on_commit} />);
  return { on_commit };
}

async function open_editor(measurement: Measurement = ONE_CUP) {
  const { on_commit } = setup(measurement);
...
describe("MeasurementEditor — unit selector conversion", () => {
  ...
  it("switching from oz to lb converts the fraction (16 oz → 1 lb)", async () => {
    const sixteen_oz: Measurement = { value: make_fraction(16, 1), unit: "oz" };
    await open_editor(sixteen_oz);
...
describe("MeasurementEditor — OK with best unit conversion", () => {
  it("OK on 48 tsp converts to 1 cup", async () => {
    const { on_commit } = await open_editor(ONE_CUP);
    await userEvent.selectOptions(screen.getByRole("combobox", { name: "Measurement unit" }), "tsp");
  ...
  it("OK on weight value converts to largest whole unit (16 oz → 1 lb)", async () => {
    const { on_commit } = setup(ONE_OZ, vi.fn());
    await userEvent.click(screen.getByRole("button", { name: "Edit measurement" }));
    await userEvent.click(screen.getByRole("radio", { name: "×" }));
    await userEvent.click(screen.getByRole("button", { name: "×2" }));
    // now 2 oz
  ...
  it("OK on count value passes through unchanged", async () => {
    const { on_commit } = await open_editor(WHOLE);
    await userEvent.click(screen.getByRole("button", { name: "OK" }));
    expect(on_commit).toHaveBeenCalledWith(
  ...
  it("fractional cup value on OK converts to fl_oz (1/2 cup = 4 fl_oz)", async () => {
    const { on_commit } = await open_editor(HALF_CUP);
    await userEvent.click(screen.getByRole("button", { name: "OK" }));
